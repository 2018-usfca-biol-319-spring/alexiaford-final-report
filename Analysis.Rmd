...
Figure 3. Mean Elevation for Six of the NEON sites
```{r elevation-table}
...
Figure 6. Temperature across all NEON sites
```{r eval}
...
aggregate_temperature %>%
  group_by(siteID) %>%
  summarize(mean_temp = mean(bioTempMean, na.rm = TRUE))
```
...
aggregate_temperature %>%
  ...
  filter(month(nameOfYourDateColumn) %in% c(5, 6, 7, 8))
```
